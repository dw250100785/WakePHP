...
class CmpAccount extends Component {
		...
		return function ($authEvent) {
			$authEvent->component->onSessionRead(function ($sessionEvent) use ($authEvent) {
				...
				if (isset($authEvent->component->req->attrs->session['accountId'])) {
					$authEvent->component->appInstance->accounts->getAccountById($authEvent->component->req->attrs->session['accountId'],
						function ($account) use ($authEvent, $cb) {
							if (!$account) {
	...
	public function UsernameAvailablityCheckController() {
		...
		$this->appInstance->accounts->getAccountByUnifiedName($username,
					...
					$req->appInstance->accounts->saveAccount(array(
						...
						function ($lastError) use ($req, $email, $password, $location, $code) {
							if ($location !== '') {
								...
								$req->components->GMAPS->geo($location,
									function ($geo) use ($req, $email) {
							...
							$req->appInstance->accounts->getAccountByUnifiedEmail($email,
								function ($account) use ($req, $password, $code) {
									if (!$account) {
										$req->setResult(array('success' => false));
										return;
									}
									$req->appInstance->Sendmail->mailTemplate('mailAccountConfirmation',
																			  $account['email'],
																			  array(
																				   'email'    => $account['email'],
																				   'password' => $password,
		...
		$this->onAuth(function ($result) use ($req) {
			...
			$job      = $req->job = new ComplexJob(function ($job) {
				...
				if (sizeof($errors) === 0) {
					...
					$req->appInstance->accounts->saveAccount($update,
						function ($lastError) use ($req, $password, $location) {
							if ($location !== '') {

								$req->components->GMAPS->geo($location,
									function ($geo) use ($req) {
	...
	public function ManageAccountsController() {
		...
		$this->onAuth(function ($result) use ($req) {
			...
			$job('countFiltered',
				function ($jobname, $job) use ($where, $limit) {
			...
			$job('find',
				function ($jobname, $job) use ($where, $sort, $fields, $fieldNames, $field, $offset, $limit) {
	...
	public function ManageAccountsDeleteController() {
		...
		$this->onAuth(function ($result) use ($req) {
			...
			$req->appInstance->accounts->deleteAccount(array('_id' => Request::getString($req->attrs->request['id'])),
		...
		$this->onSessionStart(function ($authEvent) use ($req) {
			...
			if (isset($req->attrs->request['email'])) {
				...
				if ($code !== '') {
					...
					$req->appInstance->accounts->getAccountByUnifiedEmail($email,
						function ($account) use ($req, $email) {
							if (!$account) {
								$req->setResult(array('success' => false, 'errors' => array('email' => 'Account not found.')));
								return;
							}
							$req->appInstance->accountRecoveryRequests->getLastCodeByEmail($email,
								function ($result) use ($req, $email) {
									...
									if (0) { //$result['ts'] + 900 > time()) {
										...
										$req->appInstance->Sendmail->mailTemplate('mailAccountAccessRecovery',
																				  $email,
																				  array(
																					   'email'    => $email,
																					   'password' => $password,
	...
	public function onSessionReadEvent() {
		...
		return function ($sessionEvent) {
			...
			$sessionEvent->component->appInstance->sessions->getSessionById($sid,
				function ($session) use ($sessionEvent) {
					if ($session) {
