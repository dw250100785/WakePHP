<!DOCTYPE html>
<html lang="{$req->locale}" {if isset($ng_app)} ng-app="{$ng_app|escape}"{/}>
<head>
	...
	{if $req->locale != 'en'}
		...
		{if in_array('Superusers',$req->account.aclgroups)}
			<link href="/locale/{$req->locale}/ACP.json" lang="{$req->locale}" rel="gettext"/>
		{/if}{/if}
...
<body>
<div class="block" id="{$block->parentNode->_id}">
