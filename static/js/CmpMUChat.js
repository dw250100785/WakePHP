$.chat =
{
 ...
 "acceptPM": function(bool)
 {
  ...
  {
   if ($.chat.tags[k] == '%private') {delete $.chat.tags[k];}
  }
  if (bool) {$.chat.tags.push('%private');}
  ...
  $.chat.query({"cmd":'getAvailTags'},function(o)
  {
   ...
   for (var k in o.tags)
   {
    ...
    .click(function()
    {
     ...
     $('#room .messages').html('');
     if ($.inArray('%private',$.chat.tags)) {$.chat.tags = [$(this).attr('name'),'%private'];}
     else {$.chat.tags = [$(this).attr('name')];}
     $.chat.setTags();
 ...
 "updatedUserlist": function()
 {
  ...
  for (var k in $.chat.userlist)
  {
   ...
   $('#room .userlist').append($('<p>').append(
    $('<a href="#" title="'+title+'" name="'+$.xmlescape(k)+'" onclick="$.chat.setRecipient('+$.xmlescape($.toJSON(k))+'); return false">').html($.xmlescape(k))
   .attr('_id',k)
  ...
  if (!found)
  {
   if ($.chat.tags.toString() != '') {$('#room .userlist').html("<i>No users</i>");}
   else {$('#room .userlist').html("<i>You\'re not listed on any tags.</i>");}
  }
  else
  {
 ...
 "addressOnChange": function()
 {
  ...
  $('.messages').each(function() {$(this).scrollTo('100%',{"axis": "y"});});
 },
...
<br /><br />Sent/Received data: <span class="sentDataCounter">~</span>/<span class="recvDataCounter">~</span>\
 ...
 "init": function()
 {
   ...
   $.chat.onResize($(window).width(),$(window).height());
   $.address.init(function(event) {}).change($.chat.addressOnChange);
   $('#tabs').tabs({
 ...
 "addMsg": function(o)
 {
  ...
  while ($(roomId+' .messages p').size() > 200) {$(roomId+' .messages p:first').remove();};
 },
 "sendMessage": function(o)
 {
  ...
  if (o.tags.length > 1)
  {
   for (var k in o.tags)
   {
    if (o.tags[k] == '%private') {delete o.tags[k];}
   }
  }
  $.chat.sendPacket(o);
 },
 ...
 "connect": function()
 {
  ...
  $.chat.ws.onopen = function()
  {
   ...
   if ($.chat.authkey != null) {$.chat.sendHello($.chat.authkey);}
   $.chat.su = false;
   setInterval(function()
   {
    $.chat.keepalive();
  ...
  $.chat.ws.onmessage = function(e)
  {
   ...
   else if (o.type == 'youAreModerator')
   {
    $.chat.su = true;
   }
   else if (o.type == 'availableTags')
   {
