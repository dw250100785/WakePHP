$.chat = {
	...
	"updatedUserlist": function() {
		...
		for (var k in $.chat.userlist)	{
			...
			var statuses = '';
			for (var j in u) {
				if (u[j].statusmsg != null) {
					statuses += htmlspecialchars(u[j].statusmsg)+' ... ';
				}
			}
			if (statuses != '') {
				title += ' - '+statuses;
		...
		if (!found) {
			if ($.chat.tags.toString() != '') {
				$('#userlist').html("<i>No users tagged '"+htmlspecialchars($.chat.tags.toString())+"'.</i>");
			}
			else {
				$('#userlist').html("<i>You\'re not listed on any tags.</i>");
			}
		}
		else {
			$('#userlist a').tooltip({
	...
	"initInputForm": function() {
		...
		$('#inputForm').submit(function() {
			setTimeout(function() {
				...
				if ($('#inputMessage').val() == '/') {
					if ($.chat.cmdHistory.length > 0) {
						$('#inputMessage').val($.chat.cmdHistory[$.chat.cmdHistory.length-1]);
					}
					return;
				}
				var packet = {};
	...
	"initLoginForm": function() {
		$('#inputUsername').keyboard('esc',function (event) {
			$('#inputUsername').val('');
		});
	...
	"addMsg": function(o) {
		...
		if (o.mtype == 'status') {
			...
			if ((o.to != null) && (o.to.indexOf($.chat.username) != -1)) {
